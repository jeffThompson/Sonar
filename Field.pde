...
class Field {
  ...
  final color fillColor = color(255, 100);   // dot color
  ...
  Field() {
    fieldBeep.setVolume(0.05, 0.05);    // set volume, for convenience and so I can find it again later :)
    prevMillis = millis();
  }
  ...
  void display() {
    noStroke();
    fill(fillColor);
    for (int ty = -height; ty < height*2; ty += spacing) {
